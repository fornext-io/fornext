...
// value for ChoiceState.
func (s *ChoiceState) UnmarshalJSON(data []byte) error {
	s.Type = StateTypeChoice
	s.InputPath = "$"
	s.OutputPath = "$"

	type ChoiceStateForUnmarshal ChoiceState
	return json.Unmarshal(data, (*ChoiceStateForUnmarshal)(s))
}
...
func (s *ChoiceState) ApplyInput(ctx context.Context, sc StateContext) ([]byte, error) {
...
func (s *ChoiceState) ApplyOutput(ctx context.Context, sc StateContext) ([]byte, error) {
